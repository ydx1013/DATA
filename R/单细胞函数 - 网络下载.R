...
#' 产生一个两种颜色的高亮选定细胞的图像
#' 
#' 目前这个函数只是高亮显示一组预定义的细胞。未来它还应该可以灵活地绘制数据或元数据在一组定义的高亮细胞中。
#' 
#' @param cells 一个包含细胞ID的向量，对应于dim.reduction中的行名
#' @param plot.title 字符串，图像的标题
#' @param dim.reduction 例如UMAP数据框，行名应该是细胞ID
#' @param highlight.color 字符串，指定一种颜色
#' @param aspectratio 图像的长宽比
#' @param point.size 图像中点的大小，传递给ggplot2
highlightCells <- function(cells, 
  ...
  gg <- ggplot(pl.df, aes_string(x = dim1, y = dim2, color = 'Highlight')) +
    ...
    theme(aspect_ratio = aspectratio,
  ...
  print(gg)
  # todo: 将UMAP绘图分离为另一个更灵活的函数
}

#' 从SingleCellExperiment或Seurat对象中绘制基因表达的函数，比Seurat更易于使用和更灵活。使用ggplot2。
#' 
#' 接受基因向量和SCE对象以及其他参数，并生成表达图。可以直接生成PDF文件。
#' 
#' @param genes 一个基因向量
#' @param object 一个SingleCellExperiment对象（也可能是Seurat对象）
#' @param object.class 对象的类型。"sce"或"seurat"，默认为"sce"
#' @param projection 如果您的对象只是一个矩阵，则为坐标投影
#' @param assay.name 在Seurat对象中，使用的Assay名称
#' @param embedding 在Seurat对象中，嵌入名称
#' @param convert 是否将基因符号转换为ENSEMBL ID
#' @param plot.title 图的标题
#' @param guide.name 图例标题（ggplot2）
#' @param plot.type 是否绘制一个基因、基因列表的平均值或每个基因的单独面板（目前使用ggplot2的facet，将来可能使用cowplot的网格）
#' @param w 基因表达平均值的权重向量。例如，簇成员得分。如果为NULL，则自动计算加权平均值。
#' @param scaled 是否在细胞之间进行基因表达的标准化
#' @param color.palette 颜色向量，传递给scale_color_gradientn()
#' @param aspectratio 每个图/面板的期望长宽比
#' @param rastr 是否使用ggrastr绘图（在Cairo存在问题的情况下，与aspectratio和服务器RStudio的兼容性较差）
#' @param point.size 自定义点的大小
#' @param trim 对于绘制大型数据集，修剪最高和最低的百分位数，例如c(0.01, 0.99)修剪最高和最低百分位数，使颜色刻度不受异常值的影响
#' @param lims X/Y坐标的限制。子集您的投影空间
#' @param print 是否打印绘图
#' @param num.panel.rows 对于面板绘图，有多少行的面板？
